    ...
    public class OperationBuilder
    {
        ...
        private List<Stack<IModelType>> BuildResponses(Method method, CompositeType headerType)
        {
            ...
            foreach (var response in _operation.Responses)
            {
                ...
                else if (response.Value.Extensions.TryGetValue("x-ms-treat-as-default", out treatResponseAsDefault) && ((bool?)treatResponseAsDefault).GetValueOrDefault())
                {
                    continue;
                }
                else
